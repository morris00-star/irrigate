...
class SensorData(models.Model):
    temperature = models.FloatField(null=True, blank=True)  # Allow NULL
    humidity = models.FloatField(null=True, blank=True)  # Allow NULL
    ...
    # Use the module-level constant for the default
...
class Threshold(models.Model):
    """
    Model to store historical moisture threshold changes for irrigation.
    (Consider if this is distinct from UserPreference's current threshold)
    """
    # Use the module-level constant for the default
    threshold = models.IntegerField(default=DEFAULT_THRESHOLD)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank=True)
...
class UserPreference(models.Model):
    """
    Model to store persistent user configuration preferences.
    """
    CROP_CHOICES = [
        ('banana', 'Banana'),
        ('maize', 'Maize'),
    ...
    # This stores the user's currently preferred threshold
    ...
    # Optional: Timestamps for when preferences were set/updated
    ...
    def recommended_threshold(self):
        """Returns recommended threshold based on crop and soil type"""
        if not self.crop_type or not self.soil_type:
            return DEFAULT_THRESHOLD

        # Define recommended thresholds for different crop-soil combinations
        recommendations = {
            # Banana
            'banana': {
                'clay': 45,
    ...
    def get_threshold_suggestion(self):
        """Returns explanation for the recommended threshold"""
        if not self.crop_type or not self.soil_type:
...
class DeviceStatus(models.Model):
    ...
    status_data = models.JSONField(default=dict)  # Stores full status snapshot

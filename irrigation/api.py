...
def receive_sensor_data(request):
    """
    Enhanced API endpoint with detailed logging and WebSocket support
    """
    ...
    if request.method == 'POST':
        try:
            ...
            sensor_data = SensorData.objects.create(
                temperature=clean_value(data.get('temperature')),
                humidity=clean_value(data.get('humidity')),
                moisture=clean_value(data.get('moisture')),
                pump_status=data.get('pump_status', False),
                valve_status=data.get('valve_status', False),
            ...
            # Update cache
            cache_keys = {
                f'temperature_{user.id}': sensor_data.temperature,
                f'humidity_{user.id}': sensor_data.humidity,
                f'moisture_{user.id}': sensor_data.moisture,
                f'pump_state_{user.id}': 'on' if sensor_data.pump_status else 'off',
                f'valve_state_{user.id}': 'open' if sensor_data.valve_status else 'closed',
                ...
                async_to_sync(channel_layer.group_send)(
                    f"sensor_updates_{user.id}",
                    {
                        ...
                        "data": {
                            ...
                            "valve_status": sensor_data.valve_status,
...
def control_system(request):
    ...
    try:
        ...
        def create_control_command(pump=None, valve=None, manual=None, emergency=None):
            # Ensure all required fields have defaults
            return ControlCommand.objects.create(
                pump_status=pump if pump is not None else False,  # Default to False
                valve_status=valve if valve is not None else False,  # Default to False
                manual_mode=manual if manual is not None else False,  # Default to False
                emergency=emergency if emergency is not None else False,  # Default to False
                user=user
            )
        ...
        if action == 'toggle_pump':
            ...
            # Only allow valve toggle in manual mode
            current_mode = cache.get(f'system_mode_{user_id}', False)
            if not current_mode:
                logger.warning("[VALVE] Attempted toggle while not in manual mode")
                return Response(
                    {"error": "System must be in manual mode to control valve"},
                    status=status.HTTP_403_FORBIDDEN
                )
            # Verify the requested state is different from current state
            requested_state = request.data.get('state', False)
            current_valve_state = cache.get(f'valve_state_{user_id}', 'closed')
            requested_state_str = 'open' if requested_state else 'closed'

            if current_valve_state == requested_state_str:
                logger.warning(f"[VALVE] Already in requested state: {requested_state_str}")
                return Response({"valve": current_valve_state})
            # Only proceed if state is actually changing
            cache.set(f'valve_state_{user_id}', requested_state_str, timeout=None)
            create_control_command(valve=requested_state)
            logger.info(f"[VALVE] State changed to {requested_state_str}")
            return Response({"valve": requested_state_str})
            ...
            valve_state = cache.get(f'valve_state_{user_id}', 'closed') == 'open'
            emergency_state = cache.get(f'emergency_{user_id}', False)
            # When switching to auto mode, ensure pump and valve are off
            if not manual_mode:
                pump_state = False
                valve_state = False
                cache.set(f'pump_state_{user_id}', 'off', timeout=None)
                cache.set(f'valve_state_{user_id}', 'closed', timeout=None)
                logger.info("[MODE] Switched to auto mode - ensuring pump/valve are off")
            ...
            return Response({
                ...
                "valve": "closed" if not manual_mode else ("open" if valve_state else "closed")
            })
            ...
            ControlCommand.objects.create(
                ...
                pump_status=False,
                valve_status=False,
                manual_mode=False,  # Explicitly set manual_mode
                user=user
            )
            ...
            if not current_emergency:
                ...
                return Response({
                    ...
                    "valve": cache.get(f'valve_state_{user_id}', 'closed')
                })
            cache.set(f'emergency_{user_id}', False, timeout=None)
            ...
            response_data = {
                ...
                "irrigation_active": False,  # Default to inactive
            ...
            if response_data['manual_mode'] and not response_data['emergency']:
                response_data['irrigation_active'] = any([
                    response_data['pump'] == 'on',
                    response_data['valve'] == 'open'
                ])
...
def get_system_status(request):
    ...
    try:
        ...
        return Response({
            "pump": cache.get(f'pump_state_{user_id}', 'off'),
            "valve": cache.get(f'valve_state_{user_id}', 'closed'),
            "temperature": format_value(latest_data.temperature if latest_data else None),
            "humidity": format_value(latest_data.humidity if latest_data else None),
